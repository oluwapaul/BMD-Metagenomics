...
$ cd ./unbinned
$ ./gmhmmp metaspades_500_scaff.fa -m ./MetaGeneMark_v1.mod -f G -a -d -A metaspades_500_scaff.prot.fasta -D metaspades_500_scaff.nucl.fasta
...
Again, for clarity the reference assembly these mappings were generated with is “metaspades_500_scaff.fa”
...
write.table(rpkm_round, "../rpkm_count_table_pseudocounts_20220415.txt", sep = "\t", quote = F, row.names = T)
```
...
You can use R’s formula notation to express any experimental design that can be described within an ANOVA-like framework. Note that DESeq2 uses the same kind of formula as in base R, e.g., for use by the lm function. If the question of interest is whether a fold change due to treatment is different
across groups, for example across patients, “interaction terms” can be included using models such as ∼ patient + treatment + patient:treatment. More complex designs such as these are covered in the other DESeq2 vignette.
